...
define([
    ...
    return declare("customgame.game", ebg.core.gamegui, {
        ...
        setupNotifications() {
            ...
            this._notifications.forEach((notif) => {
                ...
                this.notifqueue.setIgnoreNotificationCheck(
                    notif[0],
                    (notif) =>
                        notif.args.ignore && notif.args.ignore == this.player_id
                );
            });
        ...
        startActionTimer(buttonId, time, pref, autoclick = false) {
            ...
            this._actionTimerFunction = () => {
                ...
                if (button == null) {
                    ...
                    button.innerHTML =
                        this._actionTimerLabel +
                        " (" +
                        this._actionTimerSeconds +
                        ")";
                } else {
            ...
            this._actionTimerFunction();
            this._actionTimerId = window.setInterval(
                this._actionTimerFunction.bind(this),
                1000
            );
        ...
        initPreferencesObserver() {
            dojo.query(".preference_control, preference_fontrol").on(
                "change",
                (e) => {
                    var match = e.target.id.match(
                        /^preference_[fc]ontrol_(\d+)$/
                    );
                    if (!match) {
                        return;
                    }
      ...
      `;
        ...
        setupSettings() {
            ...
            dojo.connect($("show-settings"), "onclick", () =>
                this.toggleSettings()
            );
            this.addTooltip(
                "show-settings",
            ...
            if (this.getSettingsSections) {
                ...
                Object.keys(this._settingsSections).forEach(
                    (sectionName, i) => {
                        dojo.place(
                        ...
                        let openSection = () => {
                            dojo.query(
                                "#settings-controls-header div"
                            ).removeClass("open");
                            div.classList.add("open");
                            dojo.query(
                                "#settings-controls-wrapper div.settings-section"
                            ).removeClass("open");
                            $(`settings-section-${sectionName}`).classList.add(
                                "open"
                            );
                        };
            ...
            Object.keys(this._settingsConfig).forEach((settingName) => {
                ...
                if (config.type == "pref") {
                    ...
                    dojo.place(
                        $("preference_control_" + config.prefId).parentNode
                            .parentNode,
                        localContainer
                    );
                    return;
                ...
                let defaultValue =
                    typeof config.default === "function"
                        ? config.default(this.isMobile(), this.isTouchDevice)
                        : config.default;
                let value = this.getConfig(
                    this.game_name + suffix,
                    defaultValue
                );
                ...
                // Select type => create a select
                    ...
                    $("setting-" + settingName).addEventListener(
                        "change",
                        () => {
                ...
                // Switch type => create a select
                    ...
                    $("setting-" + settingName).addEventListener(
                        "change",
                        () => {
                            let newValue = $("setting-" + settingName).checked
                                ? 1
                                : 0;
                            this.changeSetting(settingName, newValue);
                            if (config.attribute) {
      ...
      */
            ...
            Object.keys(this._registeredCustomTooltips).forEach((id) => {
                if ($(id)) {
                    this.addCustomTooltip(
                        id,
                        this._registeredCustomTooltips[id],
                        { forceRecreate: true }
                    );
                }
            });
        ...
        addCustomTooltip(id, html, config = {}) {
            ...
            let getContent = () => {
                ...
                if (config.midSize) {
                    content =
                        '<div class="midSizeDialog">' + content + "</div>";
                }
                return content;
            };
            ...
            let updateStatus = () => {
                ...
                if (
                    ((config.optional === false &&
                        selectedElements.length == config.n) ||
                        (config.optional === true &&
                            selectedElements.length <= config.n)) &&
                    config.confirmBtn
                ) {
                    let otherElems = elemIds.filter(
                ...
                elemIds.forEach((id) => {
                    ...
                    if (config.class != "")
                        elt.classList.toggle(config.class, selected);
                    elt.classList.toggle(
                        "selectable",
                        selected || selectedElements.length < config.n
                    );
                });
